  ...
  return (
    <nav
      className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${
        scrolled
          ? "bg-[#FDF8F2]/95 backdrop-blur shadow-sm border-b border-earth-100"
          : "bg-[#FDF8F2]"
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between gap-6">
          ...
          {links.map(({ href, label }) => (
            <Link
              ...
              className={`text-sm font-medium transition-colors ${
                pathname === href
                  ? "text-earth-700 border-b-2 border-earth-500 pb-0.5"
                  : "text-bark/70 hover:text-earth-700"
              }`}
            >
              {label}
            </Link>
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          {user ? (
            <>
              <span className="text-sm text-bark/70 font-medium">
                Hi, {user?user:"User"}
              </span>
        ...
        <div className="md:hidden bg-[#FDF8F2] border-t border-earth-100 px-6 pb-5 pt-3 space-y-3">
          ...
          {links.map(({ href, label }) => (
            <Link
              ...
              className="block text-bark/80 font-medium py-1 hover:text-earth-700"
            >
              {label}
            </Link>
          ))}
          ...
          >
            ...
            <button
              type="submit"
              className="px-4 py-2 rounded-full bg-earth-600 text-cream text-sm font-medium"
            >
              Go
            </button>
          </form>
          ...
          {!user ? (
            <button
              ...
              className="w-full py-2 rounded-full bg-earth-700 text-cream text-sm font-medium"
            >
              Sign in
            </button>
          ) : (
            <button
              onClick={handleLogout}
              className="w-full py-2 rounded-full border border-earth-300 text-earth-700 text-sm font-medium"
            >
              Log out
            </button>
          )}
